...
- `npm run build`: pass (build succeeds; existing ESLint runtime/plugin issue remains: `react/display-name` rule loader mismatch)
...
- API split smoke checks:
  ...
  - `GET /api/v1/checkin/me?run_id=test` => `401` (auth required)
