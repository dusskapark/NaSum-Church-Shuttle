...
const downloadBlobStore = new Map<string, DownloadBlobEntry>();

function parseSlug(params: { slug?: string[] }) {
  return params.slug ?? [];
}

function pruneExpiredDownloadBlobs(now = Date.now()) {
  for (const [token, entry] of downloadBlobStore.entries()) {
    if (entry.expiresAt <= now) {
      downloadBlobStore.delete(token);
    }
  }
}

async function handleAdminDownloadBlobToken(request: NextRequest) {
  ...
  pruneExpiredDownloadBlobs();
  const token = randomUUID().replace(/-/g, '');
  const expiresAt = Date.now() + 10 * 60 * 1000;
  downloadBlobStore.set(token, {
    data: bytes,
...
async function handleDownloadBlob(request: NextRequest, token?: string) {
  if (request.method !== 'GET') return error(405, 'Method not allowed');
  if (!token) return error(404, 'Not found');

  pruneExpiredDownloadBlobs();
  const entry = downloadBlobStore.get(token);
...
async function handleAuthSession(request: NextRequest) {
  ...
  try {
    ...
    const message =
      caught instanceof Error ? caught.message : 'LINE auth failed';
...
async function handleUserRegistrationPost(request: NextRequest) {
  ...
  const result = await withTransaction(async (client) => {
    ...
    return { registrationId: upserted.id, userId };
  });
...
async function handleAdminSchedules(
  request: NextRequest,
  scheduleId?: string,
  suffix?: string[],
) {
...
async function routeRequest(request: NextRequest, params: { slug?: string[] }) {
  ...
  if (slug[0] === 'downloads' && slug[1]) {
    return handleDownloadBlob(request, slug[1]);
  }
  ...
  if (slug[0] === 'admin') {
    if (
      slug[1] === 'download-tokens' &&
      slug[2] === 'blob'
    ) {
      return handleAdminDownloadBlobToken(request);
    }
    if (slug[1] === 'users') {
    ...
    if (slug[1] === 'schedules') {
      return handleAdminSchedules(request, slug[2], slug.slice(3));
    }
